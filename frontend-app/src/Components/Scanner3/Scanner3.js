...
export default function Scanner3() {
    const API_URL = "http://localhost:5083/api/scanner3";
    ...
    const validateForm = () => {
        ...
        if (isEmpty(formData.scanner3Brand)) newErrors.scanner3Brand = "Brand is required";
        if (isEmpty(formData.scanner3Model)) newErrors.scanner3Model = "Model is required";
        if (isEmpty(formData.scanner3Type)) newErrors.scanner3Type = "Type is required";
        if (isEmpty(formData.scanner3Resolution)) newErrors.scanner3Resolution = "Resolution is required";
        if (isEmpty(formData.scanner3AssetTag)) newErrors.scanner3AssetTag = "Asset Tag is required";
        if (isEmpty(formData.purchaseDate)) newErrors.purchaseDate = "Purchase Date is required";
        if (isEmpty(formData.scanner3Location)) newErrors.scanner3Location = "Location is required";
    ...
    const fetchItems = useCallback(
        async (currentPage = 1) => {
            ...
            try {
                ...
                console.error("Error fetching scanner2", err);
            } finally {
                setLoading(false);
            }
        },
    ...
    const handleAssetTagChange = async (value) => {
        setFormData((prev) => ({ ...prev, scanner3AssetTag: value }));
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingId) {
                ...
                alert("✅ Scanner3(OMR Scanner) updated successfully");
            } else {
                await axios.post(API_URL, payload);
                alert("✅ Scanner3(OMR Scanner) added successfully");
            }
    ...
    const handleEdit = (p) => {
        setEditingId(p.id);
        setFormData({
            scanner3Brand: p.scanner3Brand,
            scanner3Model: p.scanner3Model,
           scanner3Type: p.scanner3Type,
            scanner3Resolution: p.scanner3Resolution,
            scanner3AssetTag: p.scanner3AssetTag,
            purchaseDate: p.purchaseDate?.split("T")[0] || "",
            scanner3Location: p.scanner3Location,
            remarks: p.remarks,
            damageReasonInput: "",
        });
        setShowForm(true);
    ...
    const exportToPDF = () => {
        ...
        autoTable(doc, {
            ...
            body: items.map((p, i) => [
                ...
                p.scanner3Model,
                p.scanner3Type,
                p.scanner3Resolution,
                p.scanner3AssetTag,
                p.scanner3Location,
            ]),
        });

        doc.save(`scanner3_${Date.now()}.pdf`);
    };

    return (
        <div className="scanner3-page container-fluid mt-4 mb-5 px-2">
            <h3 className="text-center mb-4">📠 Scanner3(OMR Scanner)</h3>
            ...
            {showForm && (
                <form className="card p-3 shadow-sm mb-4" onSubmit={handleSubmit}>
                    <h5 className="text-center fw-bold mb-3">
                        {editingId ? "✏️ Edit scanner3(OMR Scanner)" : "🆕 Add scanner3(OMR Scanner)"}
                    </h5>
                    ...
                    <div className="row g-3">
                        <div className="col-md-4">
                            ...
                            <input
                                type="text"
                                name="scanner3Brand"
                                value={formData.scanner2Brand}
                                onChange={handleChange}
                                className={`form-control ${errors.scanner3Brand ? "is-invalid" : ""}`}
                            />
                            {errors.scanner3Brand && <div className="text-danger small">{errors.scanner3Brand}</div>}
                        </div>
                        ...
                        <div className="col-md-4">
                            ...
                            <input
                                type="text"
                                name="scanner3Model"
                                value={formData.scanner3Model}
                                onChange={handleChange}
                                className={`form-control ${errors.scanner3Model ? "is-invalid" : ""}`}
                            />
                            {errors.scanner3Model && <div className="text-danger small">{errors.scanner3Model}</div>}
                        </div>
                        ...
                        <div className="col-md-4">
                            ...
                            <select
                                name="scanner3Type"
                                value={formData.scanner3Type}
                                onChange={handleChange}
                                className={`form-select ${errors.scanner3Type ? "is-invalid" : ""}`}
                            >
                            ...
                            {errors.scanner3Type && <div className="text-danger small">{errors.scanner3Type}</div>}
                        </div>
                        ...
                        <div className="col-md-4">
                            ...
                            <select
                                ...
                                value={formData.scanner3Resolution}
                                onChange={handleChange}
                                className={`form-select ${errors.scanner3Resolution ? "is-invalid" : ""}`}
                            >
                            ...
                            {errors.scanner3Resolution && <div className="text-danger small">{errors.scanner3Resolution}</div>}
                        </div>
                        ...
                        <div className="col-md-4">
                            ...
                            <input
                                type="text"
                                name="scanner3AssetTag"
                                value={formData.scanner3AssetTag}
                                onChange={(e) => handleAssetTagChange(e.target.value)}
                                className={`form-control ${errors.scanner3AssetTag || assetError ? "is-invalid" : ""}`}
                            />
                            {errors.scanner3AssetTag && <div className="text-danger small">{errors.scanner3AssetTag}</div>}
                        ...
                        <div className="col-md-4">
                            ...
                            <input
                                type="text"
                                name="scanner3Location"
                                value={formData.scanner3Location}
                                onChange={handleChange}
                                className={`form-control ${errors.scanner3Location ? "is-invalid" : ""}`}
                            />
                            {errors.scanner3Location && <div className="text-danger small">{errors.scanner3Location}</div>}
                        </div>
            ...
            {loading ? (
                ...
                <div className="alert alert-info text-center">No scanner3 (omr scanners) records found</div>
            ) : (
                <>
                            ...
                            <tbody>
                                {items.map((p, i) => (
                                    <tr key={p.id}>
                                        ...
                                        <td>{p.scanner3Brand}</td>
                                        <td>{p.scanner3Model}</td>
                                        <td>{p.scanner3Type}</td>
                                        <td>{p.scanner3Resolution}</td>
                                        <td>{p.scanner3AssetTag}</td>
                                        <td>{p.scanner3Location}</td>
